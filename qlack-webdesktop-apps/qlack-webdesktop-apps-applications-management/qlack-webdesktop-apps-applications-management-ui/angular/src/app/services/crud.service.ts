...
export class CrudService<T> {
  ...
  getAll(queryString?: string): Observable<QPageableReply<T>> {
    if (queryString) {
      ...
      return this.http.get<QPageableReply<T>>(`${this.contextPath}` + `/${this.endpoint}`);
    }
  }
  ...
  upload(form: FormGroup): Observable<any> {
    return this.qForms.uploadForm(this.http, form,
      `${this.contextPath}` + AppConstants.API_ROOT + '/apps/', false);
  }

  search(term: string, endpoint: string) {
    return this.http.get<any[]>(`${this.contextPath}` + `${AppConstants.API_ROOT}/${endpoint}/search/` + term)
    .pipe(
      map(
        (data: any) => {
          return (
